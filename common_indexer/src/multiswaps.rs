...
use crate::{event_db::Trade, AccountId};
...
fn aggregate_per_extrinsic(trades: Vec<Trade>) -> Vec<MultiSwap> {
    ...
    while start_ind < trades.len() {
        ...
        while end_ind < trades.len() && trades[end_ind].token_in == trades[end_ind-1].token_out && trades[end_ind].amount_in == trades[end_ind-1].amount_out {
            end_ind += 1;
        }
        ...
        let multiswap = MultiSwap {
            ...
            path,
            amount_in: trades[start_ind].amount_in,
            amount_out: trades[end_ind-1].amount_out,
