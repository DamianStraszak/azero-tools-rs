...
impl From<RawEvent> for Event {
	fn from(raw_event: RawEvent) -> Self {
		Self {
			...
			extrinsic_index: raw_event.extrinsic_index,
			data: raw_event.data,
		}
	}
}
...
pub fn insert_events_for_block(events: Vec<Event>, block_num: u32) -> Result<(), DbError> {
    ...
    for event_non_raw in events {
		let event: RawEvent = event_non_raw.into();
        tx.execute(
            "INSERT INTO events (
                contract_account_id_raw, 
                block_num, 
                event_index, 
                extrinsic_index, 
                data
...
pub fn get_events_by_contract(
    ...
    let mut stmt = conn.prepare(
        "SELECT contract_account_id_raw, block_num, event_index, extrinsic_index, data 
         FROM events 
    ...
    while let Some(row) = rows.next()? {
        ...
        let extrinsic_index: Option<u32> = row.get(3)?;
        let data: Vec<u8> = row.get(4)?;

        total_size += data.len() + 32 + 4 + 4 + 4;

        events.push(RawEvent { 
            contract_account_id_raw: array32, 
            block_num, 
            event_index, 
            extrinsic_index, 
            data 
