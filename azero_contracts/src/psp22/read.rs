...
use azero_config::{AccountId, RpcClient};

pub async fn read_total_supply(api: &RpcClient, contract_address: &AccountId) -> ReadFor<u128> {
...
pub async fn read_decimals(api: &RpcClient, contract_address: &AccountId) -> ReadFor<u8> {
	let instance: psp22_wrapper::Instance =
		ink_primitives::AccountId::try_from(contract_address.as_ref()).unwrap().into();
	read_from_contract(api, instance.token_decimals()).await
}

pub async fn read_name(api: &RpcClient, contract_address: &AccountId) -> ReadFor<Option<String>> {
	let instance: psp22_wrapper::Instance =
		ink_primitives::AccountId::try_from(contract_address.as_ref()).unwrap().into();
	read_from_contract(api, instance.token_name()).await
}

pub async fn read_symbol(api: &RpcClient, contract_address: &AccountId) -> ReadFor<Option<String>> {
	let instance: psp22_wrapper::Instance =
		ink_primitives::AccountId::try_from(contract_address.as_ref()).unwrap().into();
